...
pygame.mixer.music.load('game start.mp3')  # Replace with the path to your music file

# Load sound effects
#start_sound = pygame.mixer.Sound('game start.mp3')  # Replace with the path to your start sound effect file
game_over_sound = pygame.mixer.Sound('background.mp3')  # Replace with the path to your game over sound effect file
...
def get_player_name():
    ...
    input_background = pygame.transform.scale(input_background, (SCREEN_WIDTH, SCREEN_HEIGHT))  # Scale the image to match the screen size
    ...
    while not done:
        ...
        screen.blit(input_background, (0, 0))  # Blit the input background image onto the screen
...
def move_enemies():
    ...
    for car in enemy_cars:
        ...
        if car[1] > SCREEN_HEIGHT:
            ...
            score += 10  # Increase score when the enemy car goes off the screen
...
def increase_difficulty():
    ...
    new_spawn_time = max(500, spawn_time - 10)  # Gradually decrease the spawn interval but not less than 500ms
...
# Inside the main game loop where game_over is set to True:
if check_collision():
